 ...
 * 
 * Tests the Gambler driver.  Extends Maze in order to override the redraw methods since those are not used in the tests.  We also 
 * set the seed of the random number generator so that the maze will generate the same way each time.
 *
 */
public class GamblerTest extends Maze {
	...
	/**
	 * Tests that the Gambler driver can get a robot to the exit of a small deterministic maze, where it will return true.  It also tests the driver on 
	...
	public void testDrive2Exit() {
		...
		GamblerTest maze = new GamblerTest();
