 ...
 * 
 * Tests the Wizard driver.  Extends Maze in order to override the redraw methods since those are not used in the tests. 
 *
 */
public class WizardTest extends Maze {
...
public boolean increasePercentage(int pc) {
	...
	/**
	 * Tests that the Wizard driver can get a robot to the exit of a small deterministic maze, where it will return true.  It also tests the driver on 
	...
	public void testDrive2Exit() {
		...
		WizardTest maze = new WizardTest();
		...
		WizardTest maze2 = new WizardTest();
		MazeBuilder mz2 = new MazeBuilder(true);
		mz2.build(maze2, skill_x[7], skill_y[7], skill_rooms[7], skill_partct[7]);
		try {
			mz2.buildThread.join(60000);
		} catch (InterruptedException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
		maze2.mo = new Wizard(maze2);
		maze2.walle = new BasicRobot(maze2);
		try {
			maze2.mo.setRobot(maze2.walle);
		} catch (UnsuitableRobotException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
		try {
			finish = maze2.mo.drive2Exit();
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public void testGetEnergyConsumption() {
		...
		assertTrue(maze.mo.getEnergyConsumption() == 95.0);
	}
	...
	public void testGetPathLength() {
		...
		assertTrue(maze.mo.getPathLength() == 14);
	}
}
